                ...
                try {
                    ...
                    if (stripeSub && stripeSub.items && stripeSub.items.data && stripeSub.items.data.length > 0) {
                        ...
                        if (user) {
                            ...
                            console.log(`Subscription activated via verifySession for user: ${user._id}`);
                        }
                    }
                } catch (syncError) {
                    // Log error but don't fail the verification - webhook will handle it eventually
                    console.error('Error syncing subscription in verifySession:', syncError);
                }
            }
