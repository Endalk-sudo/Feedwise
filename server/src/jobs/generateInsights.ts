...
export function startInsightGenerationJob(): void {
  ...
  logger.info(
    '✅ Insight generation schedule registered (BullMQ preferred, cron fallback, distributed lock)',
  );
}
...
export function startDigestJob(): void {
  ...
  logger.info(
    '✅ Digest schedule registered (7AM, BullMQ preferred, inline fallback, distributed lock)',
  );
}
...
export function startRetentionForecastJob(): void {
            ...
            if (
              ...
              await prisma.insight.create({
                data: {
                  ...
                  action:
                    'Work the High retention-risk queue this week: Accept/Resolve the top rows first.',
                  priority: 9,
  ...
  logger.info(
    '✅ Retention forecast schedule registered (2:30AM, predictive churn + promoters, distributed lock)',
  );
}
